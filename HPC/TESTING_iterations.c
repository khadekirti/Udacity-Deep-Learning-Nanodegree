...
TESTING ITERATION vs Python Code
Weights, Bias are set to zero
*/ 
...
clock_t begin = clock();
	...
	*/ 
    ...
    // weights
    int layers[number_hlayers]=  {784, 15, 10};
    ...
    // forward propogation 
 	double kdash;
	double hideenlayerSigmoid[layers[1]];
	double hideenlayer[layers[1]];
	double output[layers[2]];
	double outputSigmoid[layers[2]]; 
	
	// Backward propogation 
	double deltaOutput[layers[2]];
 	double deltaweight2[layers[2]][layers[1]]; 
 	double deltaHidden[layers[1]];
    ...
    */ 
	...
	for(i = 0; i < layers[1]; i++){
		for(j = 0; j < layers[0]; j++){
			weight1[i][j] = 0;//sample_normal_distribution();
		}
	}

	for(i = 0; i < layers[2]; i++){
		for(j = 0; j< layers[1]; j++){
			weight2[i][j] = 0; //sample_normal_distribution(); 
		}
	}

	for(i=0; i < layers[1]; i++){
		bias1[i] = 0; //sample_normal_distribution();
	}

	for(i = 0; i < layers[2]; i++){
		bias2[i] = 0; //sample_normal_distribution();
	} 
	    ...
	    for(iterate = 0; iterate < row_train; iterate = iterate + 10){ 
		    ...
		    for ( i=0; i<layers[1]; i++) {
		   	...
		   	for( index_train = 0 ; index_train < 10; index_train++){
				...
				for(i=0;i < layers[1] ;i++){
					kdash = 0.0; 
					for(j=0;j < layers[0]; j++){
						kdash += weight1[i][j] * mini_batch_train_label[index_train][j];
					}
					hideenlayer[i] = kdash + bias1[i];
					hideenlayerSigmoid[i] = sigmoid(hideenlayer[i]);
				}
				for(i=0; i < layers[2]; i++){
					kdash = 0.0;
					for(j=0; j<layers[1]; j++){
						kdash += weight2[i][j]*hideenlayerSigmoid[j];
					}
					output[i] = kdash + bias2[i];
				...
				for(i=0; i< layers[2]; i++){
					for(j=0; j < layers[1]; j++){
						deltaweight2[i][j] += deltaOutput[i]* hideenlayerSigmoid[j];
					}
				}
			    ...
			    for ( i = 0; i< layers[1]; i++) {
			        ...
			        for( j=0; j< layers[2]; j++) {
			        	kdash += deltaOutput[j]* weight2[j][i];  
			        }
			        deltaHidden[i] += kdash * dSigmoid(hideenlayer[i]);
			    }


			    for ( i=0; i<layers[1]; i++) {
			        for ( j=0; j<layers[0]; j++) {
			            deltaweight1[i][j] += mini_batch_train_image[index_train][j] * deltaHidden[i]; 
			...
			for (i=0; i < layers[1] ; i++) 
			{
				for(j = 0;  j < layers[0] ; j++)
				{
					...
					bias1[i] = (bias1[i]  - (lr * deltaHidden[i]/10.0)); 
			} 
			...
			for (i=0; i < layers[2] ; i++)
			{
				for(j = 0;  j < layers[1] ; j++)
				{
		 		...
		 		bias2[i] = (bias2[i]  - (lr * deltaOutput[i]/10.0));  
			}		
		} 
	    ...
	    */ 	
		...
		if(epoch % 10 == 0) {
		    ...
		    for( index_test = 0; index_test < row_test ;index_test++){
			    ...
			    printf("row_test %d\n",  row_test);
			    //double accuracy = number_test_passed * 1.0/row_test;
			    //printf("accuracy %f\n", accuracy );  
			}  
 }
clock_t end = clock();
